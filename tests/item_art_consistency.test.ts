...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: '5a485049e9954ea5658f0dafae82cc469161749272994f77e89825786a262fb8',
        acceptedBytes: 113_281,
      },
    ]);
  ...
  it('pins the final 817-item visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(113_281);
    expect(sha256(verdictBytes)).toBe(
      '5a485049e9954ea5658f0dafae82cc469161749272994f77e89825786a262fb8',
    );
    ...
    expect(verdict.auditScope).toMatchObject({
      ...
      itemArtFilesReviewed: 857,
      liveItemDefinitions: 872,
    ...
    expect(verdict.visualVerdict).toMatchObject({
      ...
      reject: [],
      summary:
        'All 857 shipping item-art files pass the visual contract: 817 reviewed in the 2026-08-09 campaign (documented retries included), plus the five class-overhaul integration additions owner-reviewed and passed on 2026-08-10, plus the Dawnhold posy addition (project-authored vector illustration) owner-reviewed and passed on 2026-08-12, plus the two Proving Shore prop renders (rendered from their own shipped world models) owner-reviewed and passed on 2026-08-17, plus the three pearl-detour icons (generated via the OpenAI proving-shore-mother-of-pearl-2026-08-20 batch) owner-reviewed and passed on 2026-08-20, plus the Proving Shore Passing Stone render (rendered from its own shipped world model by the same deterministic pipeline as the 2026-08-17 pair) added on 2026-08-22, machine-checked and awaiting owner visual review, plus the nineteen buddy-whistle icons (rendered from their own shipped buddy GLB models by scripts/assets/render_buddy_item_icons.mjs) added on 2026-08-28, machine-checked and awaiting owner visual review, plus the three vendor-companion whistle icons (rendered from their own shipped buddy GLB models by scripts/assets/render_buddy_item_icons.mjs) added on 2026-09-03, machine-checked and awaiting owner visual review. The Dragon buddy whistle was removed from the game on 2026-09-04; its icon left the catalog with it. Seven more buddy icons (the six common beast and undead companions plus the epic Crystal Lich) were rendered from their own shipped rigs by scripts/assets/render_buddy_item_icons.mjs and added on 2026-09-04, machine-checked and awaiting owner visual review.',
    });
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(oldGeneratedIds).toHaveLength(543);
    const allCurrentOwnerIds = [
      ...mapping.entries.map(({ itemId }) => itemId),
      ...mapping.generatedBatches.flatMap(({ itemIds }) => itemIds),
    ];
    expect(allCurrentOwnerIds).toHaveLength(857);
    expect(new Set(allCurrentOwnerIds).size).toBe(857);
  ...
  it('keeps the full 817-icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 857) violations.push(`mapping owner count: ${ownerIds.length} != 857`);
    if (fileIds.length !== 857) violations.push(`shipping WebP count: ${fileIds.length} != 857`);
    for (const id of ids) {
